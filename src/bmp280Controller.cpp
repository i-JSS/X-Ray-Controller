...
float bmp280Controller::getTemperature(char *data) {
  ...
  double var2 = ((((double)adc_t) / 131072.0 - ((double)converter[0]) / 8192.0) *
                 (((double)adc_t) / 131072.0 - ((double)converter[0]) / 8192.0)) *
                ((double)converter[2]);
...
float bmp280Controller::getPressure(char *data) {
...
bmp280Data bmp280Controller::readData() {
  char data[8] = {0};
  i2c.read(0xF7, data, 8, 1);
  return (bmp280Data){getTemperature(data), getPressure(data)};
}
